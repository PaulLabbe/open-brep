/**
 * @file pificxml.cpp METHODES ASSOCIEES A LA CLASSE PI_FICHIER_IMPORT_XML
 */
...
bool PI_FICHIER_IMPORT_XML::s_lib_xerces_initialisees = false;

/**
 * Construire un objet permettant l'analyse syntaxique d'un fichier 
 * .xml, et par defaut, commencer l'analyse syntaxique.
 * 
 * @param nomFichier : nom du fichier 
 */
PI_FICHIER_IMPORT_XML::PI_FICHIER_IMPORT_XML(const string& nomFichier)
   :  PI_FICHIER_IMPORT( nomFichier, "",0)
{
   // Initialisation de la librairie Xerces
   // WARNING : ce Mecanisme d'initialisation/desinitialisation de la 
   // librairie Xerces n'est pas "thread-safe"
   if (!s_lib_xerces_initialisees)
   {
      try
      {
         XMLPlatformUtils::Initialize();
         s_lib_xerces_initialisees = true;
      ...
      catch (XMLException& ex)
      {
         ...
         nouveauMsg << "Erreur lors de l'initialisation de xerces\n"
            "Message : " << XML_TO_STD_STR(ex.getMessage())
             << endl;
...
/**
 *  Analyse la syntaxe du fichier .xml tout en instanciant les objets
 *  dans le gestionnaire
 *
 *  @ret int : valeur de retour
 */
